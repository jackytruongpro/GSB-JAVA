...
public class V_RecherchePraticien extends JPanel {
    ...
    public V_RecherchePraticien(V_Frame menu) {
        ...
        search_button.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                DAO_Praticien tPraticien = (DAO_Praticien) DAO_Factory.getDAOPraticien();

                try {
                    // On recup�re nos objet trouv� dans la bdd � l'aide d'un tableau*/
                    ArrayList<Praticien> result = tPraticien.findP(search_field.getText());
                    ...
                    if(result.isEmpty()){
                        ...
                        for(int i = 0; i < result.size(); i++){
                            // On creer un objet[] et on lui met en parametre l'incr�mentation pour pouvoir les mettre l'une
                            // Apres l'autre, puis on r�cup�re les diff�rent champ
                            model.insertRow(i,new Object[]{result.get(i).getNum(),result.get(i).getNom(),result.get(i).getPrenom(),
                                    result.get(i).getAddrs(),result.get(i).getCP(),result.get(i).getVille(),
                                    result.get(i).getLabCode(),result.get(i).getSecCode()});
                        }
                        //Afficher les nouvelle ligne dans notre tableau courrant
                        result_zone.setViewportView(table);
                    ...
                    table.addMouseListener(new MouseAdapter() {
                        public void mousePressed(MouseEvent me) {
                            ...
                            int ligne = Integer.parseInt(result.get(row).getNum());

                            if (me.getClickCount() == 2) {
                                View_Controller.id = result.get(row).getNum();
                                View_Controller.row = ligne;
